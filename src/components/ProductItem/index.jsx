...
function ProductItem({
  ...
  const offerPrice = useMemo(() => {
    const hasOffer = Number(item?.offerprice);
    if (hasOffer) return Number(item.offerprice);
    if (showDiscount && !isNaN(originalPrice)) {
      return originalPrice * (1 - Number(item?.discount) / 100);
    }
    return originalPrice;
  }, [item?.offerprice, item?.discount, originalPrice, showDiscount]);
  ...
  // Estado Quick View (local)
  const [quickOpen, setQuickOpen] = useState(false);
  const [quickItem, setQuickItem] = useState(null);

  const handleOpenQuick = useCallback((product) => {
    setQuickItem(product);
    setQuickOpen(true);
  }, []);

  const handleCloseQuick = useCallback(() => setQuickOpen(false), []);

  // ---- estilos MUI estabilizados (evita recreación por render) ----
  const fabSx = useMemo(
    () => ({
      minWidth: 42,
      width: 42,
  ...
  const ctaSx = useMemo(
    () => ({
      ...
      bgcolor: "transparent",
      boxShadow: "none",
      "&:hover": { borderColor: "grey.400", bgcolor: "rgba(0,0,0,0.02)" },
    }),
    []
  );
  ...
  const handleViewDetails = useCallback(
    (e) => {
      e.stopPropagation();
      // elegí una estrategia: navegar (rápido/limpio)
      navigate(`/productdetails/${item?.id}`, { state: item });
      // si también querés notificar al padre:
      onQuickView(item);
    },
    [navigate, item, onQuickView]
  );
  ...
  const handleToggleFav = useCallback(
    ...
    [isLoggedIn, item, toggleWishlist]
  );
  ...
  }, [handleOpenQuick, isLoggedIn, isOutOfStock, item]);
  ...
  return (
    ...
    >
          ...
          >
            <span>
              <Button
                ...
                disabled={!isLoggedIn}
                className="pointer-events-auto"
                sx={{ ...fabSx, color: isFav ? "error.main" : "text.primary", opacity: !isLoggedIn ? 0.5 : 1 }}
              >
      ...
      <div className="flex flex-col gap-2 px-4 py-4 bg-white flex-1">
        {/* Marca */}
        {item?.brand && (
          <div className="text-xs tracking-wide text-gray-500 uppercase">{item.brand}</div>
        )}
        ...
        <div className="mt-1 min-h-[44px] flex items-end justify-between">
          <div className="flex flex-col">
            {showDiscount && currencyAR(originalPrice) ? (
              <span className="text-xs text-gray-500 line-through">{currencyAR(originalPrice)}</span>
            ) : (
              <span className="text-xs text-transparent select-none leading-4">.</span>
            )}

            {currencyAR(offerPrice) ? (
              <span className="text-xl font-bold text-gray-900">{currencyAR(offerPrice)}</span>
            ) : (
          ...
          >
            <span>
              <Button
                ...
                sx={{
                  ...ctaSx,
                  color: !isLoggedIn || isOutOfStock ? "text.disabled" : "text.primary",
                  opacity: !isLoggedIn ? 0.5 : 1,
                }}
              >
                {!isLoggedIn ? "Iniciá sesión" : isOutOfStock ? "Sin stock" : "Agregar al carrito"}
              </Button>
            </span>
          </Tooltip>
        </div>
